/**
 * lib/ai-agent.cjs - GDN AI (Gideon) Improvement Brain (CommonJS)
 *
 * Mirrors the CrossLedger AI Organism, adapted to the GDN-AI-Gideon static site.
 * Applies SEO/copy patches to the live homepage (gdn-ai-trial.html) and
 * refreshes sitemap.xml at the repo root on each run.
 */
...
const SITE_URL = 'https://www.gdn-ai.com';
...
const IMPROVEMENT_AREAS = [
  { id: 'seo_metadata', name: 'SEO Meta Description and OG Tags', description: 'A meta description and Open Graph tags for the homepage to improve search and social previews', maxTokens: 400 },
  { id: 'hero_copy', name: 'Hero Headline and Subheading', description: 'Concise, conversion-focused hero copy describing Gideon and the free trial', maxTokens: 500 },
  { id: 'faq_content', name: 'FAQ Entry', description: 'One useful FAQ question and answer a prospective member might have', maxTokens: 600 },
];
...
async function analyzeArea(area) {
  const prompt = [
    ...
    '{ "suggestion": "the actual text to use", "rationale": "why this helps", "urgency_level": "low|medium|high" }',
  ...
  try {
    ...
    console.error('[ai-agent] Could not parse JSON for area:', area.id);
    return null;
  }
  return {
    area: area.id, areaName: area.name,
...
async function applyLivePatches(improvements) {
  ...
  try {
    ...
    if (html.includes('</head>')) {
      ...
      console.log('[ai-agent] No head close tag found - skipping SEO patch');
    }
  } catch (err) {
  ...
  try {
    ...
    const routes = ['/', '/try', '/counterparty-verification-checklist', '/sample-report', '/gdn-ai-membership-portal', '/gdn-ai-meeting-room', '/invest'];
    if (existsSync(sitemapPath) && !changed) {
      console.log('[ai-agent] No content change this run - leaving sitemap untouched');
    } else {
